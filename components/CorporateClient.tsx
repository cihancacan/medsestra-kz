...
export function CorporateClient() {
  ...
  return <>
    <section className="relative overflow-hidden bg-white px-5 pb-16 pt-32 md:px-8 md:pb-24 md:pt-36">
      <div className="absolute inset-x-0 top-0 h-[620px] bg-gradient-to-b from-[#EAF6FB] via-white to-white" />
      <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_0.72fr] lg:items-center">
        ...
        <div className="rounded-[2.4rem] bg-white p-7 shadow-2xl ring-1 ring-[#DDE8EE]"><p className="text-sm font-black uppercase tracking-[0.22em] text-[#1677A8]">Medsestra.kz</p><h2 className="mt-4 text-3xl font-black leading-[1] tracking-[-0.05em] text-[#071827]">{t.promiseH}</h2><p className="mt-5 leading-7 text-[#071827]/62">{t.promiseT}</p></div>
      </div>
    </section>
    <section className="bg-white px-5 py-20 md:px-8 md:py-28"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.85fr_1.15fr]"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-[#1677A8]">{t.benefitsEyebrow}</p><h2 className="mt-5 text-4xl font-black leading-[0.98] tracking-[-0.055em] md:text-6xl">{t.benefitsH}</h2></div><div className="grid gap-4 md:grid-cols-2">{corporateBenefits[lang].map(([title, text]) => <div key={title} className="rounded-[2rem] bg-[#F5FBFE] p-6 ring-1 ring-[#D7EEF7]"><h3 className="text-xl font-black tracking-[-0.03em] text-[#071827]">{title}</h3><p className="mt-4 leading-7 text-[#071827]/62">{text}</p></div>)}</div></div></section>
