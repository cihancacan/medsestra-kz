        ...
        <div className="flex items-center gap-2">
          <div className="relative" ref={langRef}>
            <button
              ...
              className="flex h-11 items-center gap-2 rounded-2xl bg-[#F5FBFE] px-3 text-sm font-black text-[#071827] shadow-sm ring-1 ring-[#D7EEF7]"
            >
            ...
            {isLangOpen && (
              <div className="absolute right-0 top-13 z-50 mt-2 w-44 rounded-2xl bg-white p-2 shadow-2xl ring-1 ring-[#DDE8EE]">
                {languages.map((language) => (
                  <a
                    key={language.code}
                    href={language.href}
                    onClick={() => setIsLangOpen(false)}
                    className="flex items-center justify-between rounded-xl px-3 py-2 text-sm font-bold text-[#071827] hover:bg-[#F5FBFE]"
                  >
                    <span>{language.label}</span>
                    <span className="text-[#1677A8]">{language.code}</span>
                  </a>
                ))}
              </div>
            )}
