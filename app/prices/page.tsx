  ...
  return (
    <main className="min-h-screen bg-brand-beige">
      ...
      <section className="mx-auto max-w-5xl px-5 pb-20 pt-32">
        ...
        <h1 className="mt-4 text-5xl font-semibold tracking-tight text-brand-navy md:text-7xl">Page prix</h1>
        <p className="mt-6 text-lg leading-8 text-brand-navy/70">Tarifs indicatifs. Les prix définitifs dépendent du service, de la zone, de l’horaire et du niveau d’urgence.</p>
        <div className="mt-10 overflow-hidden rounded-[2rem] bg-white shadow-premium">
          {prices.map((item, index) => (
            <div key={item.service} className={`grid gap-3 p-6 md:grid-cols-[1fr_1fr_auto] md:items-center ${index !== 0 ? 'border-t border-brand-navy/10' : ''}`}>
              <div>
                <h2 className="text-xl font-semibold text-brand-navy">{item.service}</h2>
                <p className="text-brand-blue">{item.ru}</p>
              </div>
              <p className="text-brand-navy/60">Prix affiché “à partir de”</p>
